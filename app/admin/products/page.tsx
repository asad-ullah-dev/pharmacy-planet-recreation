...
import { getAllMedicines, createMedicine, getMedicineById, updateMedicine, deleteMedicine } from "@/lib/api/medicineService"
...
export default function AdminProductsPage() {
  ...
  const [selectedProduct, setSelectedProduct] = useState<ExtendedMedicine | null>(null)
  const [isViewModalOpen, setIsViewModalOpen] = useState(false)
  const [isEditModalOpen, setIsEditModalOpen] = useState(false)
  const [isAddModalOpen, setIsAddModalOpen] = useState(false)
  const [isDeleteModalOpen, setIsDeleteModalOpen] = useState(false)
  const [medicineDetail, setMedicineDetail] = useState<ExtendedMedicine | null>(null)
  const [productToDelete, setProductToDelete] = useState<ExtendedMedicine | null>(null)
          ...
          <Dialog open={isAddModalOpen} onOpenChange={setIsAddModalOpen}>
            <DialogTrigger asChild>
              <Button
                ...
                style={{ backgroundColor: isAddModalOpen ? "#0f766e" : "#14b8a6" }}
              >
              ...
              <div className="space-y-6">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    ...
                    <p className="text-sm text-gray-900 mt-1">{medicineDetail.name}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-600">Category</Label>
                    <p className="text-sm text-gray-900 mt-1">{medicineDetail.category}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-600">Price</Label>
                    <p className="text-lg font-semibold text-gray-900 mt-1">£{medicineDetail.price.toFixed(2)}</p>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-600">Stock</Label>
                    <p className="text-sm text-gray-900 mt-1">{medicineDetail.stock} units</p>
                  </div>
                  <div className="flex flex-col gap-1">
                    <Label className="text-sm font-medium text-gray-600">Status</Label>
                    <Badge className={`${getStatusColor(medicineDetail.status)} w-fit`}>{medicineDetail.status}</Badge>
                  </div>
                  <div>
                    <Label className="text-sm font-medium text-gray-600">Total Sales</Label>
                    <p className="text-sm text-gray-900 mt-1">{medicineDetail.sales || 0} units</p>
                  </div>
                  <div className="col-span-2">
                    <Label className="text-sm font-medium text-gray-600">Description</Label>
                    <p className="text-sm text-gray-900 mt-1">{medicineDetail.description}</p>
                  </div>
                </div>
              </div>
            )}
              ...
              <div className="space-y-4">
                <div className="text-center">
                  ...
                  <p className="text-sm text-gray-600 mb-4">
                    This action cannot be undone. The product "{productToDelete.name}" will be permanently removed from your catalog.
                  </p>
                  <div className="bg-gray-50 rounded-lg p-3 mb-4">
                    <p className="text-sm font-medium text-gray-900">{productToDelete.name}</p>
                    <p className="text-xs text-gray-600">Category: {productToDelete.category}</p>
                    <p className="text-xs text-gray-600">Price: £{productToDelete.price}</p>
                  </div>
                </div>
